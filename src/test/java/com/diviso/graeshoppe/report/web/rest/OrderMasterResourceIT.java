...
public class OrderMasterResourceIT {
     ...
     * if they test an entity which requires the current entity.
     */
    public static OrderMaster createUpdatedEntity(EntityManager em) {
        OrderMaster orderMaster = new OrderMaster()
    ...
    public void getOrderMaster() throws Exception {
        ...
        restOrderMasterMockMvc.perform(get("/api/order-masters/{id}", orderMaster.getId()))
            ...
            .andExpect(jsonPath("$.storeIdpcode").value(DEFAULT_STORE_IDPCODE))
            .andExpect(jsonPath("$.storeName").value(DEFAULT_STORE_NAME))
            .andExpect(jsonPath("$.storePhone").value(DEFAULT_STORE_PHONE.intValue()))
            .andExpect(jsonPath("$.storelocationName").value(DEFAULT_STORELOCATION_NAME))
            .andExpect(jsonPath("$.methodOfOrder").value(DEFAULT_METHOD_OF_ORDER))
