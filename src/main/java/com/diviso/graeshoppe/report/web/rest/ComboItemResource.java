...
/**
 * REST controller for managing ComboItem.
 */
@RestController
@RequestMapping("/api")
public class ComboItemResource {
    ...
    /**
     * POST  /combo-items : Create a new comboItem.
     *
     * @param comboItemDTO the comboItemDTO to create
     * @return the ResponseEntity with status 201 (Created) and with body the new comboItemDTO, or with status 400 (Bad Request) if the comboItem has already an ID
    ...
    /**
     * PUT  /combo-items : Updates an existing comboItem.
     *
     * @param comboItemDTO the comboItemDTO to update
     * @return the ResponseEntity with status 200 (OK) and with body the updated comboItemDTO,
     * or with status 400 (Bad Request) if the comboItemDTO is not valid,
     * or with status 500 (Internal Server Error) if the comboItemDTO couldn't be updated
     * @throws URISyntaxException if the Location URI syntax is incorrect
     */
    ...
    public ResponseEntity<ComboItemDTO> updateComboItem(@RequestBody ComboItemDTO comboItemDTO) throws URISyntaxException {
        ...
        return ResponseEntity.ok()
            .headers(HeaderUtil.createEntityUpdateAlert(ENTITY_NAME, comboItemDTO.getId().toString()))
            .body(result);
    }

    /**
     * GET  /combo-items : get all the comboItems.
     *
     * @param pageable the pagination information
     * @return the ResponseEntity with status 200 (OK) and the list of comboItems in body
     */
    ...
    public ResponseEntity<List<ComboItemDTO>> getAllComboItems(Pageable pageable) {
        ...
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(page, "/api/combo-items");
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }

    /**
     * GET  /combo-items/:id : get the "id" comboItem.
     *
     * @param id the id of the comboItemDTO to retrieve
     * @return the ResponseEntity with status 200 (OK) and with body the comboItemDTO, or with status 404 (Not Found)
     */
    ...
    /**
     * DELETE  /combo-items/:id : delete the "id" comboItem.
     *
    ...
    /**
     * SEARCH  /_search/combo-items?query=:query : search for the comboItem corresponding
     * to the query.
     *
    ...
    public ResponseEntity<List<ComboItemDTO>> searchComboItems(@RequestParam String query, Pageable pageable) {
        ...
        HttpHeaders headers = PaginationUtil.generateSearchPaginationHttpHeaders(query, page, "/api/_search/combo-items");
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
