...
public class TicketLineResource {
     ...
     * @return the ResponseEntity with status 201 (Created) and with body the new ticketLineDTO, or with status 400 (Bad Request) if the ticketLine has already an ID
     ...
     * @return the ResponseEntity with status 200 (OK) and with body the updated ticketLineDTO,
     * or with status 400 (Bad Request) if the ticketLineDTO is not valid,
     * or with status 500 (Internal Server Error) if the ticketLineDTO couldn't be updated
     * @throws URISyntaxException if the Location URI syntax is incorrect
     */
     ...
     * @return the ResponseEntity with status 200 (OK) and the list of ticketLines in body
     */
     ...
     * @return the ResponseEntity with status 200 (OK) and with body the ticketLineDTO, or with status 404 (Not Found)
     */
    ...
    /**
     * SEARCH  /_search/ticket-lines?query=:query : search for the ticketLine corresponding
     * to the query.
     *
    ...
    public ResponseEntity<List<TicketLineDTO>> searchTicketLines(@RequestParam String query, Pageable pageable) {
        ...
        HttpHeaders headers = PaginationUtil.generateSearchPaginationHttpHeaders(query, page, "/api/_search/ticket-lines");
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
